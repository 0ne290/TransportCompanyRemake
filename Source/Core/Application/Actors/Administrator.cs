...
public class Administrator(IEntityStorageService<Entities.Driver> driverStorageService, IEntityStorageService<Entities.Truck> truckStorageService, IEntityStorageService<Entities.Branch> branchStorageService, IEntityStorageService<Entities.Order> orderStorageService, IEntityStorageService<Entities.User> userStorageService, IGeolocationService geolocationService)
{
    ...
    public async Task<IEnumerable<Dtos.Branch.Response>> GetBranches(Expression<Func<Entities.Branch, bool>> filter, bool includeTrucks, bool includeDrivers)
    {
        ...
        if (includeTrucks)
        {
            ...
            getTruckResponses = b => b.Trucks.Select(t => new Dtos.Truck.Response(t.Guid, t.CommissionedDate,
                ...
                t.VolumeMax, t.VolumePrice, t.WeightMax, t.WeightPrice, t.PricePerKm, null, null, null));
        }
        else
    ...
    public async Task<IEnumerable<Dtos.Branch.Response>> GetPotentialOrderPerformersByBranches(string orderGuid)
    {
        ...
        foreach (var branch in branches)
        {
            ...
            var trucks = branch.Trucks.Where(t => truckPredicate(t)).Select(t => new Dtos.Truck.Response(t.Guid, t.CommissionedDate,
                t.DecommissionedDate, t.PermittedHazardClassesFlags == null ? null : HazardClassesFlags.FlagCombinationToString(t.PermittedHazardClassesFlags.Value), t.Number, t.IsAvailable, t.TrailerIsTank,
                t.VolumeMax, t.VolumePrice, t.WeightMax, t.WeightPrice, t.PricePerKm, null, null, t.CalculateOrderPricePerKm(order) * (decimal)lengthInKmAndDrivingHours.LengthInKm));
    ...
    public async Task<ICollection<Dtos.Driver.ByWorkload>> AggregateAllDriversByWeeklyWorkload() =>
        (from driver in await driverStorageService.FindAll(_ => true, "PrimaryOrders;SecondaryOrders")
            let primaryOrders = driver.PrimaryOrders.Where(o => o.DateEnd >= DateTime.Now - TimeSpan.FromDays(7)).ToList()
    ...
    public async Task<ICollection<Dtos.Driver.ByWorkload>> AggregateAllDriversByTotalWorkload() =>
        (from driver in await driverStorageService.FindAll(_ => true, "PrimaryOrders;SecondaryOrders")
            let primaryOrders = driver.PrimaryOrders.Where(o => o.Status == OrderStatuses.Completed).ToList()
    ...
    public async Task<ICollection<Dtos.Driver.ByEfficiency>> AggregateAllDriversByEfficiency() =>
        (from driver in await driverStorageService.FindAll(_ => true, "PrimaryOrders;SecondaryOrders")
            let primaryOrders = driver.PrimaryOrders.Where(o => o.Status == OrderStatuses.Completed).ToList()
            let secondaryOrders = driver.SecondaryOrders.Where(o => o.Status == OrderStatuses.Completed).ToList()
            let priceOfAllOrders = primaryOrders.Sum(o => o.Price!.Value) + secondaryOrders.Sum(o => o.Price!.Value)
    ...
    // public async Task<ICollection<Dtos.Driver.ByWorkload>> AggregateAllDriversByWeeklyWorkload()
    // {
    //     var drivers = await driverStorageService.FindAll(_ => true, "PrimaryOrders;SecondaryOrders");
    //     var ret = new List<Dtos.Driver.ByWorkload>();
    //     foreach (var driver in drivers)
    //     {
    //         var primaryOrders = driver.PrimaryOrders.Where(o => o.Status == OrderStatuses.Completed).ToList();
    //         var secondaryOrders = driver.SecondaryOrders.Where(o => o.Status == OrderStatuses.Completed).ToList();
    //         ret.Add(new Dtos.Driver.ByWorkload(driver.Name, primaryOrders.Count + secondaryOrders.Count,
    //             primaryOrders.Sum(o => o.ActualHoursWorkedByDriver1!.Value) +
    //             secondaryOrders.Sum(o => o.ActualHoursWorkedByDriver2!.Value)));
    //     }
    // 
